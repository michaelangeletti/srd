...
Examples (local disk-to-disk):
    script.py /Volumes/MediaDrive/digreq-2664 /Users/mangelet/Desktop/copy --local
    script.py /Volumes/MediaDrive/digreq-2664 /Volumes/BackupDrive/copy --local --resume
"""
...
def main():
    """Entry point for the srd command."""
    global USE_LOCAL, USE_PUSH, USE_CSV, USE_COMPRESSION, USE_RESUME, USE_CREATE_DEST
    global REMOTE_DIR, LOCAL_DIR, LOG_FILE, TREE_FILE, CSV_FILE
    global LOG_DIR, IS_MACOS, REMOTE_HOST_ALIAS, SSH_TIMEOUT, RSYNC_TIMEOUT
    global MD5_CHUNK_SIZE, PROGRESS_DOT_INTERVAL, PROGRESS2_RE
    ...
    if not IS_MACOS and '--open-ssh' in sys.argv[1:]:
        ...
        print()
        try:
            subprocess.run(['ssh', '-M', '-S', socket_path, '-o', 'ControlPersist=yes', '-N', f'{REMOTE_USER}@{REMOTE_HOST_ALIAS}'], check=True)
    ...
    v0.1 -- January 2026 ({_platform_label})
    ========================="""
    ...
    def run_ssh_command(command: str, timeout: int = SSH_TIMEOUT) -> Optional[str]:
        ...
        if IS_MACOS:
            ...
            ssh_cmd = ['ssh', '-o', 'ControlMaster=no', '-o', f'ControlPath={CONTROL_SOCKET}', f'{REMOTE_USER}@{REMOTE_HOST_ALIAS}', command]
    ...
    def check_ssh_socket() -> bool:
        """Ubuntu only: verify the ControlMaster socket exists."""
        if IS_MACOS:
            return True
    ...
    def run_rsync_transfer(src: str, dst: Path, total_bytes: int = 0, remote_dst: str = '') -> Tuple[int, float]:
        ...
        rsync_cmd = ['rsync', '-av8', '--protect-args', '--exclude=.*', '--info=progress2', '--timeout=300']
        if IS_MACOS:
            rsync_cmd.insert(3, '--iconv=UTF-8-MAC,UTF-8')
        if not IS_MACOS and not USE_LOCAL:
    ...
    """
        ...
        try:
            process = subprocess.Popen(
                (['ssh', REMOTE_HOST_ALIAS, verify_cmd] if IS_MACOS
                 else ['ssh', '-o', 'ControlMaster=no', '-o', f'ControlPath={CONTROL_SOCKET}', f'{REMOTE_USER}@{REMOTE_HOST_ALIAS}', verify_cmd]),
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                errors='surrogateescape',
    ...
    def replicate() -> bool:
        ...
        if USE_PUSH:
            ...
            dst_remote = f"{REMOTE_HOST_ALIAS}:{LOCAL_DIR}" if IS_MACOS else f"{REMOTE_USER}@{REMOTE_HOST_ALIAS}:{LOCAL_DIR}"
            ...
            src = f"{REMOTE_HOST_ALIAS}:{REMOTE_DIR}" if IS_MACOS else f"{REMOTE_USER}@{REMOTE_HOST_ALIAS}:{REMOTE_DIR}"
            return_code, duration = run_rsync_transfer(src, local_path, source_size_bytes)
