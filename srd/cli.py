...
if IS_MACOS:
    ...
    LOG_DIR        = "/home/YOUR_LOCAL_USERNAME/srd_logs"  # ← change this
...
def check_ssh_socket() -> bool:
    """Ubuntu only: verify the ControlMaster socket exists before any SSH call."""
    if IS_MACOS:
        return True
    ...
    logger.error("The server requires Duo two-factor authentication.")
    logger.error("Open a persistent SSH connection first by running:")
    logger.error("")
    logger.error("    srd --open-ssh")
    logger.error("")
    logger.error("Approve the Duo prompt, then run srd in a separate terminal.")
    return False
...
def run_rsync_transfer(src: str, dst: Path, total_bytes: int = 0, remote_dst: str = '') -> Tuple[int, float]:
    ...
    # macOS: normalize HFS+ Unicode filenames (not available on Linux rsync)
    if IS_MACOS:
        rsync_cmd.insert(3, '--iconv=UTF-8-MAC,UTF-8')
    # Ubuntu: route rsync through the ControlMaster socket
...
def replicate() -> bool:
    ...
    if USE_PUSH:
        ...
        dst_remote = (f"{REMOTE_HOST_ALIAS}:{LOCAL_DIR}" if IS_MACOS
                      else f"{REMOTE_USER}@{REMOTE_HOST_ALIAS}:{LOCAL_DIR}")
        ...
        src = (f"{REMOTE_HOST_ALIAS}:{REMOTE_DIR}" if IS_MACOS
               else f"{REMOTE_USER}@{REMOTE_HOST_ALIAS}:{REMOTE_DIR}")
        return_code, duration = run_rsync_transfer(src, local_path, source_size_bytes)
