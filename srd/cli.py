...
and validates checksums. Ubuntu 24.04 version.

Usage:
    script.py <SOURCE_DIR> <DEST_DIR> [OPTIONS]

Options:
    --local                 Replicate from a local directory (no SSH required)
    --compress, -z          Enable compression (slower but uses less bandwidth)
    --resume                Enable resume for interrupted transfers (slower)

Examples (SFTP/remote):
    script.py /pool0/smpl2/digreq-2664 /Users/mangelet/Desktop/copy
    script.py /pool0/smpl2/digreq-2664 /Users/mangelet/Desktop/copy --compress
    script.py /pool0/smpl2/digreq-2664 /Users/mangelet/Desktop/copy --resume

Examples (local disk-to-disk):
    script.py /media/MediaDrive/digreq-2664 /home/mangelet/copy --local
    script.py /media/MediaDrive/digreq-2664 /media/BackupDrive/copy --local --resume
"""
...
REMOTE_HOST_ALIAS = "sul-smpl.stanford.edu"  # SSH host alias — update if needed
REMOTE_USER       = "YOUR_SERVER_USERNAME"    # ← your username on the remote server
# ControlMaster socket path — created by running: srd --open-ssh
# %h is replaced by the hostname automatically by SSH.
CONTROL_SOCKET    = "/tmp/srd_ctl_%h"
LOG_DIR = "/home/YOUR_LOCAL_USERNAME/srd_logs"  # ← update to your local home folder
...
def print_usage():
    ...
    print(f"  {sys.argv[0]} /pool0/smpl2/digreq-2664 /Users/mangelet/Desktop/copy --resume")
    print()
...
if '--open-ssh' in sys.argv[1:]:
    # Launch a persistent ControlMaster connection so the user can complete
    # Duo authentication once, after which srd can run unattended.
    ...
    print(f"Please complete the Duo prompt. Leave this terminal open while srd runs.")
    print()
    try:
        subprocess.run([
            'ssh',
            ...
            '-o', 'ControlPersist=yes',    # Keep socket alive after this process exits
            '-N',                          # No remote command — connection only
            f'{REMOTE_USER}@{REMOTE_HOST_ALIAS}',
        ], check=True)
        print(f"\n✓ SSH session open. Socket: {socket_path}")
        print(f"  You can now run srd normally in a separate terminal.")
...
REMOTE_DIR = sys.argv[1]  # Set from command-line argument
LOCAL_DIR = sys.argv[2]   # Set from command-line argument
...
v0.1 -- January 2026 (Ubuntu 24.04)
========================="""
...
# Matches rsync --info=progress2 progress lines in both phases:
#   During initial scan:  ... (xfr#5,  ir-chk=1052/1112)
#   During transfer:      ... (xfr#5,  to-chk=245/300)
# Groups: (xfr_num, files_remaining, files_total)
...
class TransferStats:
    ...
    verified_count: int = 0          # files that passed MD5 verification
    unverifiable_count: int = 0      # files whose sidecar was missing or unreadable
...
# Print header to both console and log file
print(SCRIPT_HEADER)
...
def run_ssh_command(command: str, timeout: int = SSH_TIMEOUT) -> Optional[str]:
    ...
    """
    # Use the existing ControlMaster socket — never create a new one.
    # The socket must have been opened first with: srd --open-ssh
    SSH_OPTS = [
        '-o', 'ControlMaster=no',
        '-o', f'ControlPath={CONTROL_SOCKET}',
    ]
    ssh_cmd = ['ssh'] + SSH_OPTS + [f'{REMOTE_USER}@{REMOTE_HOST_ALIAS}', command]
...
def check_ssh_socket() -> bool:
    """
    Verify the ControlMaster socket exists before attempting any SSH operations.
    Gives a clear actionable error if it is missing.
    """
    # Expand %h in the socket path to the actual hostname
    ...
    logger.error("The server requires Duo two-factor authentication which must be")
    logger.error("completed interactively before running srd. Open a persistent")
    logger.error("SSH connection first by running:")
    logger.error(f"")
    logger.error(f"    srd --open-ssh")
    logger.error(f"")
    logger.error("Then approve the Duo prompt and leave that terminal open.")
    logger.error("You can then run srd normally in a separate terminal.")
    return False
...
def run_rsync_transfer(src: str, dst: Path, total_bytes: int = 0, remote_dst: str = '') -> Tuple[int, float]:
    ...
    # SSH options used by rsync for remote transfers — reuse the ControlMaster socket
    RSYNC_SSH = f"ssh -o ControlMaster=no -o ControlPath={CONTROL_SOCKET}"

    # Base rsync command - optimized for speed by default
    rsync_cmd = [
        'rsync',
        '-av8',              # Archive mode, 8-bit clean
        '--protect-args',
        '--exclude=.*',
        '--info=progress2',  # Better progress info
        '--timeout=300',     # Timeout after 5 min of no data
    ]

    # Apply SSH auth options for remote (SFTP) transfers only
    if not USE_LOCAL:
        rsync_cmd.extend(['-e', RSYNC_SSH])
...
def verify_remote_integrity(
    ...
    file_list_sh = " ".join(
        f"'{remote_dest_path.rstrip("/")}/{f}'" for f in data_files
    )
...
"""
    ...
    try:
        process = subprocess.Popen(
            ['ssh', '-o', 'ControlMaster=no', '-o', f'ControlPath={CONTROL_SOCKET}', f'{REMOTE_USER}@{REMOTE_HOST_ALIAS}', verify_cmd],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            errors='surrogateescape',
...
def main():
    """Package entry point — all configuration and globals are already
    initialised at module level, so this just runs the transfer."""
    try:
        success = replicate()
